...
import { type DigitalLegacyRecord, type VaultContact } from "@/lib/vault-data";
...
export default function DigitalLegacyDetailPage() {
  const params = useParams<{ id: string }>();
  const [record, setRecord] = useState<DigitalLegacyRecord | null>(null);
  ...
  useEffect(() => {
    const loadRecord = async () => {
      try {
        ...
        const nextRecord = vault?.digitalLegacy.find((item) => item.id === params.id) ?? null;
    ...
    <div className="min-h-screen bg-[#F2F2F7] font-sans text-slate-800 antialiased">
      <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col overflow-x-hidden bg-[#F2F2F7]">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between bg-[#F2F2F7]/80 px-6 py-5 backdrop-blur-lg">
          <Link
            href="/digital-legacy"
            aria-label="Back to digital legacy records"
          ...
          <h1 className="text-sm font-semibold uppercase tracking-[0.15em] text-slate-600">Digital Detail</h1>
        ...
        <main className="flex-1 space-y-4 px-6 pb-36 pt-4">
          ...
          {!isLoading && !record ? (
            <div className="glass-card rounded-3xl border border-[#e7eaee] bg-white/85 p-6 text-center">
              <p className="text-sm font-semibold text-slate-700">Digital record not found</p>
            ...
            <section className="glass-card rounded-[2rem] border border-[#e7eaee] bg-white/90 p-6 shadow-[0_16px_30px_-18px_rgba(15,23,42,0.25)]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-bold text-slate-900">{record.platform}</h2>
                </div>
                <div className="rounded-2xl bg-sky-50 px-4 py-3 text-right text-sky-700">
                  <p className="text-[10px] font-bold uppercase tracking-widest">Access</p>
                  <p className="mt-1 text-sm font-semibold">{record.accountIdentifier || record.accountPassword ? "Credentials saved" : "Reference only"}</p>
                </div>
              </div>

              <div className="mt-6 space-y-4">
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Access Where</p>
                  <p className="mt-2 text-sm text-slate-800">{record.whereToFind}</p>
                </div>

                {record.accountIdentifier || record.accountPassword ? (
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Login</p>
                      <p className="mt-2 text-sm text-slate-800">{record.accountIdentifier || "Not set"}</p>
                    </div>
                    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Password</p>
                      <p className="mt-2 text-sm text-slate-800">{record.accountPassword || "Not set"}</p>
                    </div>
                  </div>
                ) : null}
